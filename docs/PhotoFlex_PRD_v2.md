...
> 决策记录：[[PhotoFlex 开发日志]]  
> 当前阶段：Phase 0 — Prototype与技术spike结束
...
这是根据codex生成的PRD做完Protype与技术反转后的一次PRD重写（20260825），目的是以自己的语言厘清要做的一切事情。
接下来所有产品开发的大纲以本文档为准，若与以前的文档有所冲突，先与我确认再继续工作。
...
> **PhotoFlex 是为长期摄影项目设计的本地视觉编辑工作台：它不移动原片、不替用户做艺术判断，而是帮助摄影师管理项目、挖掘项目、研究项目、记录项目的灵感、更深入地观看照片、探索不同序列、攥写文字，陪伴摄影师到最终打印以前的整个项目周期。**

### 1.2 产品背景

目前市面上存在大量与照片本身相关的产品，包括照片管理、编辑、美化、输出、生成等各式各样的功能，摄影师们在应对单张照片时有着非常丰富的产品可供选择。然而，目前市面上却没有专注于帮助艺术摄影师完成一整个摄影项目的产品。长期摄影项目的材料和思考通常散落在 Lightroom/Capture One、磁盘文件夹、PPT/Milanote、纸样、PDF、聊天和个人笔记中。

对于艺术摄影来说，重要的不止是单张照片的力度，更重要的是整个照片系列的序列、凝结度，以及整体对于主题的思考与叩问。因此，多个照片的排序尝试以及创作过程中的文字思考对于一个摄影项目中十分重要。


### 1.3 核心产品假设

> 若摄影师希望能有一个整体的软件统领项目，低摩擦地对比多个序列版本，那么 PhotoFlex 会比“Lightroom + 文件夹 + PPT/Milanote”更适合长期摄影项目的视觉编辑阶段。

这是假设，不是已证实结论。
> 验证方法：上线一个比Miro白板好用的照片序列工具，让摄影群里的人用，看使用的人数、进行对比并做记录的人数等。
重点是观看的方法。
...
1. 一个以**摄影项目**为核心驱动的工作台，整合关于这个项目所需的一切（照片、研究材料、灵感记录等）
2. 一个专为摄影项目而设的项目管理工具
3. 更方便尝试、对比各种照片序列的工具
3. 一个记录项目灵感、创作过程、反思的工具
4. 一个快捷查看序列在书中呈现状态的工具
...
| OBJ-05 | 完成讨论交接 | 用户能导出可阅读 dummy 和可追溯序列清单 |
