...
| Project / Contact Sheet | 添加 Source、浏览和预览照片 | 建立项目照片引用和 Pool |
| Sequence | 线性排序、网格观看、全景和大图判断 | 更新唯一工作草稿 |
| Whiteboard Sort | 自由摆放照片辅助排序 | 保存时回写工作草稿顺序 |
| Compare | 选择两个命名版本并对照 | 只读显示差异，不修改历史 |

Whiteboard Sort 是 Sequence 内的辅助视图，不是独立产品阶段，也不出现在顶层导航中。
...
flowchart TB
    ...
    Sequence --> Whiteboard[Whiteboard Sort 模块]
...
| Library | Contact Sheet、Pool、预览、照片分页 | 命名版本 |
| Sequence | 工作草稿、排序、删除、undo/redo、观看模式 | 直接读文件系统 |
| Versioning | VersionId、不可变快照、打开副本、A/B diff | 修改历史版本 |
| Compare | 选择两个版本、展示序列和差异 | 编辑版本 |
| Whiteboard Sort | 临时坐标、框选移动、回写顺序 | 便签、文字、连线、排版 |
| ProjectStore | 工作区、CAS revision、独立版本、原子事务、schema 迁移、备份/恢复和失败结果 | 照片文件读取 |
| PhotoSource | 文件夹授权、照片枚举和预览 URL | 写入或删除原片 |

跨模块接口只在代码的 `src/contracts/index.ts` 及其直接导出文件中定义一次。本文只记录模块责任和行为承诺，不维护第二套 TypeScript 签名；Sequence、Whiteboard、Versioning、适配器和测试必须导入同一份 canonical contract。M0 代码尚未建立前，以《开发技术架构方案》的契约草案为准；M0 完成后以代码为准。
...
erDiagram
    ...
    PROJECT_WORKSPACE ||--|| POOL : owns
    POOL }o--o{ PHOTO_REF : selects
    PROJECT_WORKSPACE ||--|| SEQUENCE_DRAFT : edits
    SEQUENCE_DRAFT ||--o{ SEQUENCE_ITEM : orders
...
- 文件移动或重命名后旧引用标记为 `missing-file` 并保留在 Pool/Sequence/版本中，不按文件名静默重连或删除；Source 整体失去权限才标记 `permission-lost`；
- 同一 Photo 可以在 Pool、工作草稿和多个版本中被引用；
- Sequence 中的每个位置使用独立 SequenceItemId，PhotoId 只表示照片身份；白板输入、选择、移动、删除和回写都使用 SequenceItemId，不得用 PhotoId 去重；
- Project 同一时刻只有一个可编辑 `SequenceDraft`；
- ProjectWorkspace 只保存 VersionId 列表，不嵌入历史快照；
- `SequenceVersion` 以 VersionId 为主键独立存储，保存完整顺序且不依赖后来变化的草稿；
- 版本名允许重复，只用于展示；查询、打开、比较和父版本关系全部使用 VersionId；
- 删除 Source 前必须显示其影响，第一阶段不提供删除原文件的能力；
- Source 权限失效时保留 PhotoRef、Pool 和版本位置，并显示重新授权入口。

IndexedDB 使用五个独立 object store：
...
存储同时维护三个独立版本号：IndexedDB 数据库版本控制 object store/index，ProjectWorkspace schemaVersion 控制工作区记录形状，备份 schemaVersion 控制可移植 JSON。第一版就建立顺序 migration runner；升级只在 `onupgradeneeded` 的 versionchange 事务中执行。迁移失败不删除旧库；其他标签页必须响应 `versionchange` 关闭连接，升级被阻塞时明确提示用户关闭其他 PhotoFlex 标签页。
...
保存版本前先在事务外完成快照构造和校验。ProjectStore 随后打开同时覆盖 `projects` 与 `versions` 的单个 IndexedDB `readwrite` 事务，在关联的 IndexedDB request 回调中完成 CAS 检查、写入 SequenceVersion、将 VersionId 加入 ProjectWorkspace 并增加 revision。事务内不得读取文件、解码图片、等待计时器/网络或其他无关 Promise；只有 transaction `complete` 后才报告成功，`abort/error` 时两处都回滚。

### 6.3 白板排序

```mermaid
flowchart LR
    Seq[当前线性序列] --> Open[生成临时白板布局]
    Open --> Arrange[自由移动/框选/删除]
    Arrange -->|放弃| Seq
    Arrange -->|预览顺序| Derive[按确定规则生成 SequenceItemId 顺序]
    Derive --> Preview[卡片编号 + 只读线性预览条]
    Preview -->|返回调整| Arrange
    Preview -->|确认回写| Draft[更新工作草稿]
```

白板使用固定尺寸卡片和世界坐标：按中心 Y 分行，半个卡片高度为同行阈值；行内按中心 X 排序，并依次用中心 Y、进入白板前顺序和 SequenceItemId 打破平局。完全重叠时保持原进入顺序。每次移动/删除都会让旧预览失效，用户必须看到编号和线性预览并再次确认，系统不能静默回写。白板坐标只服务本次判断；命名版本只保存最终线性顺序，因此 Compare 不需要理解画布。
...
`ProjectWorkspace.revision` 是工作区级非负单调整数：创建项目为 0，每次成功写入加 1。`saveWorkspace` 和 `createVersion` 都在写事务内先读取当前记录并执行 compare-and-swap：只有当前 revision 等于调用方的 expectedRevision 才能写入，否则不改变任何数据并返回 expected/actual conflict。
...
Undo/Redo 只在当前页面会话有效，意思是“历史栈不写入 IndexedDB”，不是“撤销后的结果不保存”。每次 Undo/Redo 都产生新的草稿状态，并与普通移动/删除命令一样进入同一个串行保存队列；如果 moveB 正在保存，undo(moveB) 排在其后，只有撤销结果落盘后才能显示“已保存”。非冲突的瞬时失败可由用户从最后成功的 revision 重试最新状态；revision conflict 必须停止队列并要求重新加载，不能盲重试。刷新后恢复最后成功保存的 SequenceDraft，同时清空撤销和重做历史。
...
| Whiteboard | 只挂载视口附近照片；框选计算使用完整轻量坐标数据 |
...
开发阶段先用 30 张固定 fixture 验证闭环，再逐步扩大到 500、2,000 和 10,000 张图库。Library 和 Pool 可以容纳大图库，但第一阶段的 SequenceDraft 和 SequenceVersion 使用 500 项硬上限。一次添加若会超过 500 项，命令整体拒绝且不部分写入，界面显示当前数量、上限和本次尝试数量；用户仍可从 Pool 改选较少照片。因为无法创建超过 500 项的版本，Compare 输入也不会超过 500 项。

两个 500 项版本的 Diff 在目标硬件上 p95 ≤200ms；10,000 张图库指标不等于支持 10,000 项序列。纯状态引擎耗时不能代替真实浏览器帧率测试。

Diff Gate 必须运行在 production build 的真实 Chrome/Chromium 页面中，记录浏览器/系统/硬件，预热 5 次后至少测量 30 次并报告 p50/p95。Vitest 或 Node 中的纯状态耗时只能用于回归。命名版本采用完整快照，项目版本存储按“版本数 × 每版项目数”线性增长；M5 额外测量 200 个版本 × 500 项时的数据库占用、Compare 打开和 JSON 备份大小。
...
桌面框架已选择 Tauri 2。正式 renderer、文件安全、数据库恢复和 macOS Gate 仍需在桌面 Alpha 前完成；若出现硬门失败，优先缩小白板或数据规模，再按 ADR 的复议条件重新评估。
...
- 用户能在不解释术语的情况下完成 Project → Source → Pool → Sequence；
...
- Sequence 第 501 项被明确拒绝且不发生部分写入，Library/Pool 仍可继续容纳大图库；
- 两个 500 项版本的 Diff 达到 p95 ≤200ms；
- 白板以 SequenceItemId 保存稳定线性顺序；同一 PhotoId 出现两次时不会去重或丢失，放弃不会改变序列；
- 白板重叠、跨行和斜排场景使用确定 tie-break；回写前展示编号与线性预览，布局变化后旧预览失效；
- 刷新后项目结构和版本可恢复；
- 项目可以导出带独立 `schemaVersion: 1` 的 JSON，并以新 ProjectId 完整恢复；缺失、未知或损坏的 schema 在任何写入前被拒绝；
- Source 权限丢失不会导致 Pool、序列或版本记录消失；
...
- IndexedDB schema 1 → 2 的 migration runner 可执行；升级失败不删库，未知新版本不降级写入；
- PreviewLease 共享 URL 时不会提前 revoke，离开可见/预取窗口并淘汰后不会泄漏；
- 自动测试覆盖 Pool → v1 → 修改 → v2 → Compare → 重开 → 备份/恢复的闭环；
- 畸形/随机备份输入返回明确错误并保持五个 object store 完全不变；
- 没有原片写入能力。
