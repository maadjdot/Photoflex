...
export function TableContextToolbar({ draft, actions, canAddToSequence, onExecute, onRequestSequence, onAddToSequence, onPreview, onComparePhotos, onCompareSequences, onRemovePiles }: TableContextToolbarProps) {
  ...
  return <div className="table-context-toolbar-position"><div className="table-context-toolbar" role="group" aria-label={t("table.selectionActions")}>
    ...
    {photoIds.length > 0 && <>
      {memberGroup && <TableToolButton icon={leaveGroupIcon} label={t("table.leaveGroup")} text={t("table.leaveGroup")} shortcut="Shift+G" title={t("table.removeOnlyFromGroup")} onClick={() => onExecute({ type: "remove-from-group", photoId: photoIds[0] })} />}
    ...
    <TableToolButton icon={frontIcon} label={t("table.front")} shortcut="F" disabled={!actions.canBringToFront} onClick={() => onExecute(pileIds.length ? { type: "bring-sequence-piles-to-front", sequenceIds: pileIds } : { type: "bring-to-front", photoIds })} />
