...
export function TableMemos({ draft, zoom, selectedId, onSelect, onExecute, disabled }: {
  ...
  return <>{(draft.memos ?? []).map((memo) => <MemoCard key={memo.id} memo={memo} draft={draft} zoom={zoom} selected={memo.id === selectedId} onSelect={() => onSelect(memo.id)} onExecute={onExecute} disabled={disabled} />)}</>;
}

function MemoCard({ memo, draft, zoom, selected, onSelect, onExecute, disabled }: {
  memo: WorktableMemo; draft: WorktableDraft; zoom: number; selected: boolean; onSelect: () => void;
  ...
  return <>
    ...
    <article className={`table-memo${selected ? " is-selected" : ""}`} aria-label="Table memo" style={{ left: shown.x, top: shown.y, width: shown.width, height: shown.height }} onPointerDown={(event) => { event.stopPropagation(); if (!disabled) onSelect(); }} onPointerMove={(event) => event.stopPropagation()} onPointerUp={(event) => event.stopPropagation()} onKeyDown={(event) => event.stopPropagation()}>
      <header><button type="button" className="memo-drag" aria-label="Move memo" title="Drag to move memo" disabled={disabled} onPointerDown={(event) => start(event, "move")} onPointerMove={move} onPointerUp={finish} onPointerCancel={(event) => finish(event, true)} onKeyDown={(event) => handleKeys(event, "move")}><span aria-hidden="true">⠿</span></button><button type="button" className="memo-delete" aria-label="Delete memo" title="Delete memo" disabled={disabled} onClick={() => onExecute({ type: "remove-memo", memoId: memo.id })}>×</button></header>
      <textarea autoFocus={selected && !memo.text} aria-label="Memo text" placeholder="Write a memo…" value={memo.text} disabled={disabled} style={{ fontSize: memo.fontSize }} onFocus={onSelect} onChange={(event) => onExecute({ type: "update-memo", memoId: memo.id, changes: { text: event.target.value } })} />
      <button type="button" className="memo-resize" aria-label="Resize memo" title="Drag to resize freely" disabled={disabled} onPointerDown={(event) => start(event, "resize")} onPointerMove={move} onPointerUp={finish} onPointerCancel={(event) => finish(event, true)} onKeyDown={(event) => handleKeys(event, "resize")} />
    </article>
  </>;
}
