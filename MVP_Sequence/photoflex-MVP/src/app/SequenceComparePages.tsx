...
export function VersionComparePage({
  ...
  return <main className="sequence-compare-page page">
    <header>
      ...
      <span>{diff.value.added.length} added · {diff.value.removed.length} removed · {diff.value.moved.length} moved · {diff.value.readingUnitChanged.length} unit</span>
    </header>
    <div className="sequence-compare-lanes">
...
function VersionLane({ label, version, added, removed, moved, dependencies }: { label: string; version: SequenceVersion; added: ReadonlySet<SequenceItemId>; removed: ReadonlySet<SequenceItemId>; moved: ReadonlySet<SequenceItemId>; dependencies: AppDependencies }) {
  const { locale, t } = useLocale();
  return <section className="sequence-compare-lane"><header><b>{label}</b><strong>{version.name}</strong><span>{t("common.itemCount", { count: version.items.length })}</span></header><div>{version.items.map((item, index) => <article key={item.id} className={added.has(item.id) ? "is-only" : removed.has(item.id) ? "is-only" : moved.has(item.id) ? "is-moved" : "is-shared"}>{item.kind === "photo" ? <PhotoThumb photoSource={dependencies.photoSource} photoId={item.photoId} alt={`${version.name} ${index + 1}`} /> : item.kind === "text" ? <div className="sequence-text-thumb" style={{ fontSize: `${Math.max(8, Math.min(18, item.fontSize * .3))}px` }}>{item.text || t("sequence.textPlaceholder")}</div> : <div className="sequence-blank-page">{t("sequence.blank")}</div>}<span>{index + 1}</span><small>{locale === "zh-CN" ? (added.has(item.id) ? "新增" : removed.has(item.id) ? "移除" : moved.has(item.id) ? "移动" : "未变化") : (added.has(item.id) ? "ADDED" : removed.has(item.id) ? "REMOVED" : moved.has(item.id) ? "MOVED" : "UNCHANGED")}</small></article>)}</div></section>;
}
...
function SequenceLane({ label, sequence, only, moved, dependencies }: { label: string; sequence: SequenceDocument; only: ReadonlySet<PhotoId>; moved: ReadonlySet<PhotoId>; dependencies: AppDependencies }) {
  const { locale, t } = useLocale();
  return <section className="sequence-compare-lane"><header><b>{label}</b><strong>{sequence.name}</strong><span>{t("common.itemCount", { count: sequence.items.length })}</span></header><div>{sequence.items.map((item, index) => { const onlyHere = item.kind === "photo" && only.has(item.photoId); const movedHere = item.kind === "photo" && moved.has(item.photoId); return <article key={item.id} className={item.kind === "blank" ? "is-blank" : onlyHere ? "is-only" : movedHere ? "is-moved" : "is-shared"}>{item.kind === "photo" ? <PhotoThumb photoSource={dependencies.photoSource} photoId={item.photoId} alt={`${sequence.name} ${index + 1}`} /> : item.kind === "text" ? <div className="sequence-text-thumb" style={{ fontSize: `${Math.max(8, Math.min(18, item.fontSize * .3))}px` }}>{item.text || t("sequence.textPlaceholder")}</div> : <div className="sequence-blank-page">{t("sequence.blank")}</div>}<span>{index + 1}</span><small>{locale === "zh-CN" ? (item.kind === "blank" ? "空白页" : item.kind === "text" ? "文字页" : onlyHere ? `仅 ${label}` : movedHere ? "移动" : "共有") : (item.kind === "blank" ? "BLANK" : item.kind === "text" ? "TEXT" : onlyHere ? `ONLY ${label}` : movedHere ? "MOVED" : "SHARED")}</small></article>; })}</div></section>;
}
