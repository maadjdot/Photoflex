...
export function TablePage({ dependencies, projectId, navigate }: { dependencies: AppDependencies; projectId: ProjectId; navigate: (route: AppRoute) => void }) {
  ...
  return <main className="table-page page">
    <TableContextToolbar projectName={workspace.name} draft={draft} actions={actions} canUndo={tableSession.canUndo} canRedo={tableSession.canRedo} canAddToSequence={Boolean(photoIds.length && summaries.length)} saving={writeSnapshot.saving} writeState={writeSnapshot.writeState} onRetrySave={() => void coordinator.retry()} onUndo={() => history("undo")} onRedo={() => history("redo")} onExecute={execute} onRequestSequence={requestSequence} onAddToSequence={() => { setAddToSequenceId(summaries[0]?.id); setAddToSequenceOpen(true); }} onPreview={setPreviewPhotoId} onComparePhotos={setComparePhotoIds} onCompareSequences={(ids) => navigate({ name: "sequence-compare", projectId, leftSequenceId: ids[0], rightSequenceId: ids[1] })} onRemovePiles={(ids) => setDeleteConfirmation(ids)} onRemovePhotos={(ids) => execute({ type: "remove", photoIds: ids })} />
    ...
    {addToSequenceOpen && <TableSequenceAddDialog coordinator={coordinator} projectId={projectId} photoIds={photoIds} summaries={summaries} initialSequenceId={addToSequenceId} onClose={() => setAddToSequenceOpen(false)} onSummariesChange={setSummaries} onNotice={setNotice} />}
    ...
    <SequenceOrderPanel coordinator={coordinator} dependencies={dependencies} projectId={projectId} sequenceId={activeSequenceId} navigate={navigate} onPhotoError={onPhotoError} onNotice={setNotice} />
    </TableWorkspace>
